...
func TestSignatureGenerate_InvalidProtocol(t *testing.T) {
	...
	if !bytes.Contains(rr.Body.Bytes(), []byte("INVALID_PROTOCOL")) {
		t.Fatalf("body %q does not contain INVALID_PROTOCOL", rr.Body.String())
	}
}

func TestSignatureGenerate_InvalidMTU(t *testing.T) {
	...
	if !bytes.Contains(rr.Body.Bytes(), []byte("INVALID_MTU")) {
		t.Fatalf("body %q does not contain INVALID_MTU", rr.Body.String())
	}
}
