...
function buildSingleTunnel(id) {
	...
	const allowedIPs = base.defaultRoute ? ['0.0.0.0/0', '::/0'] : ['10.0.0.0/8'];
	return {
		id: base.id,
		name: base.name,
		type: base.type,
		enabled: base.enabled,
		defaultRoute: Boolean(base.defaultRoute),
		ispInterface: base.resolvedIspInterface,
		ispInterfaceLabel: base.resolvedIspInterfaceLabel,
		interfaceName: base.interfaceName,
		state: base.status,
		backend: base.backend === 'nativewg' ? 'nativewg' : 'kernel',
		warnings: [],
		interface: {
			privateKey: mockPubkey(500 + Number.parseInt(base.id.replace(/\D+/g, ''), 10)),
		...
		peer: {
			publicKey: mockPubkey(600 + Number.parseInt(base.id.replace(/\D+/g, ''), 10)),
			presharedKey: '',
			endpoint: base.endpoint,
			allowedIPs,
			persistentKeepalive: 25,
		},
		connectivityCheck: base.connectivityCheck ?? { method: 'http' },
		pingCheck: {
			enabled: true,
